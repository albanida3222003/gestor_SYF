...
public class EmpleadosModel extends Conexion {
	...
	public Empleados obtenerUltimoEmpleado() {
        Empleados emp = null;
        try {
            // Llamada al procedimiento almacenado
            String sql = "call sp_obtener_ultimo_empleado()";
            this.openConnection();
            cs = conexion.prepareCall(sql);
            rs = cs.executeQuery();
            ...
            System.out.println("obtenerUltimoEmpleado() " + e.getMessage());
        }
        return emp;
    }
	...
	public int editarEmpleados(Empleados emp) {
		...
		try {
			...
			if (filasAfectadas == 0) {
				System.out.println("Edicion en empleado fallido.");
			}
		} catch (Exception e) {
