...
INSERT INTO `produccion` VALUES (1,'2024-01-10',6000,3,2,3),(2,'2024-01-10',6000,4,2,3),(3,'2024-01-11',6000,5,1,1),(4,'2024-01-11',6000,6,2,3),(5,'2024-01-12',6000,3,2,3),(6,'2024-01-12',6000,4,2,3),(7,'2024-01-13',6000,5,1,1),(8,'2024-01-13',6000,6,2,3);
...
INSERT INTO `proveedor` VALUES (1,'Plásticos del Norte','contacto@plasticosdelnorte.com','998877665','Av. Plástico 123, Lima, Perú'),(2,'Plastilux S.A.','ventas@plastilux.com','987654321','Calle Luz 456, Arequipa, Perú'),(3,'Plastimundo','info@plastimundo.com','976543210','Jr. Mundo 789, Trujillo, Perú'),(4,'TecnoPlast','ventas@tecnoplast.com','965432109','Av. Tecnología 101, Cusco, Perú'),(5,'Plastifar','contacto@plastifar.com','954321098','Calle Fábrica 212, Piura, Perú'),(6,'Plásticos Industriales','ventas@plasticosindustriales.com','933210987','Jr. Industria 345, Ica, Perú'),(7,'Plastisur','info@plastisur.com','922109876','Av. Sur 456, Tacna, Perú'),(8,'Plastimax','contacto@plastimax.com','911098765','Calle Máxima 567, Lambayeque, Perú'),(9,'Plastiplast','ventas@plastiplast.com','900987654','Av. Plastificación 678, Arequipa, Perú'),(10,'Plastiquera del Oeste','info@plastiqueraoeste.com','889876543','Jr. Oeste 789, Moquegua, Perú');
...
BEGIN
    SELECT idconsumidor, nombre, telefono, correo, direccion
    FROM consumidor
    WHERE idconsumidor = idConsumidor;
END ;;
DELIMITER ;
...
BEGIN
    SELECT idcontrato, fecha_comienzo, fecha_fin, vigencia, id_empleado, id_departamento
    FROM contrato
    WHERE idcontrato = idContrato;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_obtener_departamento_por_id` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_obtener_departamento_por_id`(
    IN p_iddepartamento INT
)
BEGIN
    SELECT iddepartamento, nombre, salario, horas_semanales
...
BEGIN
    SELECT iddetalle_suministro, tipo_producto, cantidad, precio, total_coste, stock, id_proveedor
    FROM detalle_suministro
    WHERE iddetalle_suministro = idDetalleSuministro;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    SELECT idempleado, nombre, apellido, dni, sexo, correo, telefono, direccion, fecha_nacimiento
    FROM empleado
...
BEGIN
    SELECT idproduccion, fecha_produccion, cantidad, id_empleado, id_producto, id_detalle_suministro
    FROM produccion
    WHERE idproduccion = idProduccion;
END ;;
DELIMITER ;
...
BEGIN
    SELECT idproducto, capacidad_ml, color, material
    FROM producto
...
BEGIN
    SELECT idproveedor, nombre, correo, telefono, direccion
    FROM proveedor
    WHERE idproveedor = idProveedor;
END ;;
DELIMITER ;
...
BEGIN
    SELECT idventas, fecha, cantidad, precio, total_coste, id_producto, id_consumidor, id_empleado
    FROM ventas
    WHERE idventas = idVentas;
END ;;
DELIMITER ;
...
-- Dump completed on 2025-01-17 16:51:48
